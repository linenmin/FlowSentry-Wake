...
TEST_F(TransformPostambleTest, valid_onnx_file_initialization)
{
  ...
  EXPECT_NO_THROW(Transformer("libtransform_postamble.so", { { "onnx_path", onnx_path } }));
}
...
TEST_F(TransformPostambleTest, initialization_parameter_validation_errors)
{
  ...
  EXPECT_THROW(Transformer("libtransform_postamble.so", input_mismatch), std::logic_error);
  ...
  EXPECT_THROW(Transformer("libtransform_postamble.so", input_only_scale), std::logic_error);
  ...
  EXPECT_THROW(Transformer("libtransform_postamble.so", input_only_zeropoint), std::logic_error);

  // Test no dequant params (should be fine)
  std::unordered_map<std::string, std::string> input_no_dequant
      = { { "onnx_path", onnx_path } };
  EXPECT_NO_THROW(Transformer("libtransform_postamble.so", input_no_dequant));
}
...
TEST_F(TransformPostambleTest, tensor_selection_plan_validation)
{
  ...
  EXPECT_THROW(Transformer("libtransform_postamble.so", input_wrong_plan_size),
      std::runtime_error);
  ...
  std::unordered_map<std::string, std::string> input_missing_dequant
      = { { "onnx_path", onnx_path }, { "tensor_selection_plan", "1" }, // Selects tensor index 1
          ...
          { "dequant_zeropoint", "0" } };
  EXPECT_THROW(Transformer("libtransform_postamble.so", input_missing_dequant),
      std::runtime_error);
}
...
TEST_F(TransformPostambleTest, multi_input_onnx_validation)
{
  ...
  std::unordered_map<std::string, std::string> input_correct
      = { { "onnx_path", onnx_path }, { "tensor_selection_plan", "0,1" },
          { "dequant_scale", "1.0,1.0" }, { "dequant_zeropoint", "0,0" } };
  EXPECT_NO_THROW(Transformer("libtransform_postamble.so", input_correct));
  ...
  std::unordered_map<std::string, std::string> input_insufficient
      = { { "onnx_path", onnx_path }, { "tensor_selection_plan", "0,1" },
          ...
          { "dequant_zeropoint", "0" } };
  EXPECT_THROW(Transformer("libtransform_postamble.so", input_insufficient), std::runtime_error);
}

TEST(transform_postamble, null_data_pointer_error)
{
  // Test error handling during transform with null data pointer
  Transformer transformer("libtransform_postamble.so", {});
  ...
  EXPECT_THROW(transformer.transform(inp, out), std::runtime_error);
}
...
TEST(transform_postamble, passthrough_mode_empty_config)
{
  ...
  Transformer transformer("libtransform_postamble.so", input);
...
TEST_F(TransformPostambleTest, custom_thread_config)
{
  ...
  EXPECT_NO_THROW(Transformer transformer("libtransform_postamble.so", input));
  ...
  EXPECT_NO_THROW(transformer.transform(inp, out));
