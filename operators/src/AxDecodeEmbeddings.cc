...
// Optimized Seechange embeddings decoder
...
namespace seechange_embeddings
{
...
decode_to_meta(const AxTensorsInterface &in_tensors,
    const seechange_embeddings::properties *prop, unsigned int current_frame,
    ...
    const AxDataInterface &video_interface, Ax::Logger &logger)
{
  ...
  ax_utils::insert_meta<AxMetaEmbeddings>(map, prop->meta_name,
      prop->master_meta, current_frame, total_frames,
...
{
  auto props = std::make_shared<seechange_embeddings::properties>();
...
set_dynamic_properties(const std::unordered_map<std::string, std::string> &input,
    seechange_embeddings::properties *prop, Ax::Logger &logger)
{
}
